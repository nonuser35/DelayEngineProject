...
$env:GOOS = "windows"
$env:GOARCH = "amd64"
& $go.Source build -buildvcs=false -trimpath -ldflags="-s -w" -o ".\dist\DelayEngine.exe" ".\cmd\delayengine"
if ($LASTEXITCODE -ne 0) {
    throw "Build falhou."
}

Write-Host "Compilando dist\DelayEngineTray.exe..."
& $go.Source build -buildvcs=false -trimpath -ldflags="-s -w -H=windowsgui" -o ".\dist\DelayEngineTray.exe" ".\cmd\delayengine-tray"
if ($LASTEXITCODE -ne 0) {
...
$SupportSource = Join-Path $ProjectRoot "support"
if (-not (Test-Path -LiteralPath $SupportSource)) {
    $SupportSource = Join-Path $RepositoryRoot "support"
}
$SupportWebDestination = Join-Path $AppDir "web\support"
if (Test-Path -LiteralPath $SupportSource) {
    New-Item -ItemType Directory -Force -Path $SupportWebDestination | Out-Null
    Copy-Item -Path (Join-Path $SupportSource "*") -Destination $SupportWebDestination -Recurse -Force
}
