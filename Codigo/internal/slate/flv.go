...
type Player struct {
	...
	// ClockStart and ClockDTS keep repeated plays on one continuous media
	// clock. Leaving them zero preserves the standalone one-shot behavior.
	ClockStart time.Time
	ClockDTS   time.Duration
}
...
// Duration returns the actual playable duration of an FLV slate. The short
// transition uses this value so it always respects the user's converted file.
func Duration(path string) (time.Duration, error) {
	file, err := os.Open(path)
	if err != nil {
		return 0, fmt.Errorf("open slate FLV: %w", err)
	}
	defer file.Close()
	...
	if !haveFirst || last <= first {
		return 0, fmt.Errorf("slate FLV has no playable duration")
	}
...
func (p *Player) Play(ctx context.Context, startDTS time.Duration, maxDuration time.Duration) (Result, error) {
	...
	var haveFirst bool
	var lastAudioDTS time.Duration
	var lastVideoDTS time.Duration
	var haveLastAudioDTS bool
	var haveLastVideoDTS bool
	var messages uint64
	startWall := p.ClockStart
	clockDTS := p.ClockDTS
	if startWall.IsZero() {
		startWall = time.Now()
		clockDTS = startDTS
	...
	for {
		...
		outDTS = monotonicMessageDTS(msg, outDTS, &lastAudioDTS, &haveLastAudioDTS, &lastVideoDTS, &haveLastVideoDTS)

		waitUntil := playbackWaitUntil(startWall, clockDTS, outDTS)
...
func playbackWaitUntil(clockStart time.Time, clockDTS, outputDTS time.Duration) time.Time {
	return clockStart.Add(outputDTS - clockDTS)
}
...
func monotonicMessageDTS(msg message.Message, dts time.Duration, lastAudio *time.Duration, haveAudio *bool, lastVideo *time.Duration, haveVideo *bool) time.Duration {
	switch msg.(type) {
	case *message.Audio:
