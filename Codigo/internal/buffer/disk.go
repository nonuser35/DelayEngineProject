...
// LatestKeyframeSnapshot reads only the most recent GOP instead of the whole
// disk buffer. The latter can be minutes long and would pause publication
// during a return-to-live transition.
func (b *Disk) LatestKeyframeSnapshot() ([]media.Packet, error) {
	b.mu.RLock()
	if len(b.entries) == 0 {
		b.mu.RUnlock()
		return nil, nil
...
// DelayedRange returns media up to latest-targetDelay. The first call starts
// on a keyframe; following calls continue from the last emitted DTS.
func (b *Disk) DelayedRange(targetDelay, after time.Duration, initial bool) ([]media.Packet, time.Duration, error) {
	if targetDelay <= 0 {
		return nil, 0, nil
	}
	b.mu.RLock()
	...
	stop := start
	for stop < len(b.entries) && b.entries[stop].Timestamp <= end {
		stop++
	}
	if stop == start {
		b.mu.RUnlock()
	...
	entries := append([]diskEntry(nil), b.entries[start:stop]...)
	snapshotDelay := latest - entries[0].Timestamp
	b.mu.RUnlock()
	packets, err := b.readEntries(entries)
	return packets, snapshotDelay, err
}

// DelayedInitialRange starts at a decodable keyframe at or before the exact
// delayed point, then includes a small amount of following media. The extra
// media is paced by the publisher; it is prebuffer, not extra viewer delay.
func (b *Disk) DelayedInitialRange(targetDelay, prefetch time.Duration) ([]media.Packet, time.Duration, error) {
	if targetDelay <= 0 {
		return nil, 0, nil
	}
	b.mu.RLock()
...
func (b *Disk) SetMaxDuration(maxDuration time.Duration) {
	if maxDuration <= 0 {
		return
	}
