...
func (t *trayApp) onReady() {
	...
	hotkeyStatus := systray.AddMenuItem("Atalhos: carregando...", "Atalhos globais configurados no painel")
	hotkeyStatus.Disable()
	...
	go t.watchHotkeySettings(hotkeyStatus)
...
func (t *trayApp) watchHotkeySettings(statusItem *systray.MenuItem) {
	settingsPath := filepath.Join(config.RuntimeRoot(), "settings.json")
	apply := func(force bool) {
		configured, err := hotkey.LoadFile(settingsPath)
		if err != nil {
			t.logger.Warn("hotkey settings not ready", "error", err, "status", "waiting")
			return
		}
		t.hotkeyMu.Lock()
		defer t.hotkeyMu.Unlock()
		if !force && configured.Arm.Canonical == t.hotkeys.Arm.Canonical && configured.Live.Canonical == t.hotkeys.Live.Canonical {
			return
		}
		if t.stopHotkeys != nil {
			t.stopHotkeys()
		}
		t.hotkeys = configured
		t.stopHotkeys = startGlobalHotkeys(t.logger, []hotkeyAction{
			{ID: 1, Modifiers: configured.Arm.Modifiers, Key: configured.Arm.Key, Description: configured.Arm.Canonical, Run: t.armDelay},
			{ID: 2, Modifiers: configured.Live.Modifiers, Key: configured.Live.Key, Description: configured.Live.Canonical, Run: t.forceRealtime},
		})
		statusItem.SetTitle(fmt.Sprintf("Atalhos: %s / %s", configured.Arm.Canonical, configured.Live.Canonical))
		t.logger.Info("global hotkeys updated without restart", "arm", configured.Arm.Canonical, "live", configured.Live.Canonical, "status", "ok")
	}
...
func (t *trayApp) openPanel() {
	...
	go func() {
		if err := waitForHTTP(url, 45*time.Second); err != nil {
			t.logger.Warn("panel not ready yet; opening browser anyway", "error", err, "status", "waiting")
		}
...
func (t *trayApp) armDelay() {
	// Arming with a loading slate waits until the visual transition and the
	// requested delay are ready. Five seconds is shorter than a normal slate,
	// so the tray used to report a false failure while the engine was healthy.
	...
	if strings.EqualFold(strings.TrimSpace(settings.DelayArmMode), "buffer") || strings.EqualFold(strings.TrimSpace(settings.DelayArmMode), "direct") {
		body, _ := json.Marshal(map[string]any{"seconds": seconds})
		response, err := client.Post(t.apiURL("/delay/arm-buffer"), "application/json", bytes.NewReader(body))
		if err != nil || response.StatusCode >= 300 {
			if response != nil {
				response.Body.Close()
			}
			t.logger.Error("failed to arm buffer delay from tray", "error", err, "status", "error")
			return
		}
		response.Body.Close()
		t.logger.Info("buffer delay armed from tray", "delay_seconds", seconds, "status", "ok")
		return
	}
	...
	shortSlate := strings.EqualFold(strings.TrimSpace(settings.DelayArmMode), "short-loading")
	body, err := json.Marshal(map[string]any{
		"seconds":       seconds,
		"slate":         videos.Active,
		"playFullSlate": !shortSlate && settings.PlayFullLoading,
		"slateMode":     map[bool]string{true: "short", false: ""}[shortSlate],
	})
	if err != nil {
	...
	if response.StatusCode >= 300 {
		message := strings.TrimSpace(readResponseMessage(response.Body))
		if message == "" {
			message = response.Status
		}
		t.logger.Error("arm delay from tray failed", "status_code", response.StatusCode, "message", message, "status", "error")
		return
	}
...
func (t *trayApp) forceRealtime() {
	request, err := http.NewRequest(http.MethodPost, t.apiURL("/delay/off"), nil)
	if err != nil {
...
func waitForHTTP(url string, timeout time.Duration) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			...
			return fmt.Errorf("panel did not become ready before timeout")
		}
