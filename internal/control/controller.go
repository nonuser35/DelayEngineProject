...
func (c *Controller) DisableDelay(ctx context.Context) error {
	return c.DisableDelayWithSlate(ctx, "", 0)
}

func (c *Controller) DisableDelayWithSlate(ctx context.Context, slatePath string, slateDuration time.Duration) error {
	...
	if slatePath != "" {
		return reader.ForceRealtimeWithSlate(ctx, slatePath, slateDuration)
	}
...
func (c *Controller) ArmDelay(ctx context.Context, delayValue time.Duration, slatePath string, playFullSlate bool) error {
	...
	return reader.ArmDelay(ctx, inputrtmp.ArmDelayRequest{Delay: delayValue, SlatePath: slatePath, PlayFullSlate: playFullSlate})
}
